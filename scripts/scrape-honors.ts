...
async function scrapeHonors(): Promise<Student[]> {
...
export async function run(lastUpdated: string, outDir: string) {
  const students = await scrapeHonors();
  fs.writeFileSync(path.join(outDir, "honors.json"), JSON.stringify({ lastUpdated, students }, null, 2), "utf-8");
  console.log(`榮譽榜完成：${students.length} 位學生，已寫入 honors.json`);
}

// 直接執行
if (process.argv[1].includes("scrape-honors")) {
